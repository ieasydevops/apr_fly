...
APR_DECLARE(apr_status_t) apr_proc_create2(apr_proc_t *new_proc, \
        const char *progname, \
        const char *const *args, \
        const char *user, \
        int login, \
        ...
        apr_pool_t *pool) {
    int i;
    new_proc->in = attr->parent_in;
    new_proc->err = attr->parent_err;
    new_proc->out = attr->parent_out;

    if ((new_proc->pid = fork()) < 0) {
        return errno;
    }
    else if (new_proc->pid == 0) {
        if (attr->child_in) {
            apr_pool_cleanup_kill(apr_file_pool_get(attr->child_in),
                                  attr->child_in, apr_unix_file_cleanup);
        }
 ...
 */
                ...
                default: {
                    ...
                    ch = onearg = (char*)apr_palloc(pool, onearg_len);
                    i = 0;
                    while (args[i]) {
                        size_t len = strlen(args[i]);
...
//{
//    if (newargs[i])
//    fwrite(newargs[i], strlen(newargs[i]), 1, fp);
//}
//fwrite("\n", 1, 1, fp);
//fclose(fp);

            execv("/bin/su", (char *const *) newargs);
        }
